package com.its.karaoke.interceptor;

import com.its.karaoke.exceptions.KaraUnauthenticated;
import com.its.karaoke.security.SecurityContexts;
import com.its.karaoke.service.JwtService;
import com.its.karaoke.utils.IPHelper;
...
@Log4j2
@Component
public class KaraInterceptor implements HandlerInterceptor {

    private JwtService jwtService;

    @Autowired
    public KaraInterceptor(JwtService jwtService) {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (Strings.isEmpty(vendorCode)) {
            throw new KaraUnauthenticated("Bạn cần đăng nhập trước khi sử dụng API này");
        }
