...
@Log4j2
public class KaraTrackLogRequestInterceptor implements HandlerInterceptor {

    private static final String START_ATTR = KaraTrackLogRequestInterceptor.class.getName() + ".startTime";
    private static final String HANDLING_ATTR = KaraTrackLogRequestInterceptor.class.getName() + ".handlingTime";
    private static final long DEFAULT_WARN_NANOS;
    private long warnHandlingNanos;

    public KaraTrackLogRequestInterceptor() {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        if (!(handler instanceof HandlerMethod)) {
            ...
            log.info("[START CONTROLLER][{}]:{}.{}({})", new Object[]{request.getSession().getId(), m.getDeclaringClass().getSimpleName(), m.getName(), buildMethodParams(handlerMethod)});
    ...
    public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler, ModelAndView modelAndView) {
        if (handler instanceof HandlerMethod) {
            ...
            if (this.isEnabledLogLevel(isWarnHandling)) {
                ...
                log.info("[END CONTROLLER][{}]:{}.{}({})-> view={}, model={}", new Object[]{request.getSession().getId(), m.getDeclaringClass().getSimpleName(), m.getName(), buildMethodParams(handlerMethod), view, model});
                ...
                if (isWarnHandling) {
                    ...
                    log.info(handlingTimeMessage, new Object[]{m.getDeclaringClass().getSimpleName(), m.getName(), buildMethodParams(handlerMethod), formattedHandlingTime});
                }
            }
        }
    }
